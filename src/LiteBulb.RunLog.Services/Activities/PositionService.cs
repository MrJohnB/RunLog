...
namespace LiteBulb.RunLog.Services.Activities
{
	...
	public class PositionService : IPositionService
	{
		...
		public ServiceResponse<IPagedResult<Position>> GetPagedList(int offset = 0, int limit = 50)
		{
			var pagedResult = _repository.GetPagedList(offset, limit);

			if (pagedResult == null)
			{
				return new ServiceResponse<IPagedResult<Position>>(true,
					$"Error occurred while retrieving paged list of {nameof(Position)} objects.  PagedResult was null for some reason.");
			}

			if (pagedResult.Data == null)
			{
				return new ServiceResponse<IPagedResult<Position>>(true,
					$"Error occurred while retrieving paged list of {nameof(Position)} objects.  PagedResult.{nameof(pagedResult.Data)} was null for some reason.");
			}

			return new ServiceResponse<IPagedResult<Position>>(pagedResult);
		}
